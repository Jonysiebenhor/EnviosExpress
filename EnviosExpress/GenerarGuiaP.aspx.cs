...
{
    public partial class GenerarGuiaP : System.Web.UI.Page

    {
        ...
        protected void Button4_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (tipo == "Estandar")
                {   
                    Tipo0.Text = "Cree una cuenta para crear guias Estandar";
                    Tipo0.ForeColor = Color.Red;
                }
                else if (tipo == "Pagar Solo Envio")
                {
                    string cantidadadepositar = "0";

                    monto = montoenvio.Text;
                    valorenvio = montoenvio.Text;
                    conectado.conectar();
                    conectado.crearguiapaquete1(remitente, destinatario, direccion, telefono, monto, peso, valorenvio, cantidadadepositar, departamento, municipio, zona,  tipo);

                    DataRow rows = conectado.Guia().Rows[0];
                    String guia = Convert.ToString(Convert.ToString(rows["idpaquete"]));
                    Response.Redirect("DescargarP.aspx?id=" + guia);
                }
                ...
                else
                {
                    ...
                    {
                        Tipo0.Text = "Cree una cuenta para crear guias Pago Contra Entrega";
                        Tipo0.ForeColor = Color.Red;
                    }
                }
            }
        ...
        protected void DropDownList5_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (ddlzona.Text == "" || ddldepartamento.Text == "" || ddlmunicipio.Text == "")
                {
